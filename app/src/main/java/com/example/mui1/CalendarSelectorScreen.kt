...
@Composable
fun CalendarSelectorScreen(currentSelectetDate : Date, onDateSelected : (Date) -> Unit, onBackClick : () -> Unit, onFutureClick: () -> Unit){
    ...
    Scaffold(modifier = Modifier.fillMaxSize()) {paddingValues ->
        Column(modifier = Modifier.fillMaxSize().padding(paddingValues), verticalArrangement = Arrangement.Center, horizontalAlignment = Alignment.CenterHorizontally) {
            Text(text = currentSelectetDate.formatToText(Locale.getDefault()))
            Spacer(modifier = Modifier.height(16.dp))

            Row(modifier = Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.CenterVertically){
                TextField(value = changeDateValue.toString(), onValueChange = { value->
